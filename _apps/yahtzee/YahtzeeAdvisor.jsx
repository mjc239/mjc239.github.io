...
// Each rule variant has its own precomputed value table, generated by
// solver/generate_values.py with the matching rules.
const MODES = {
  standard: { file: "values.bin", rules: { extraYahtzeeBonus: 100, joker: true } },
  nobonus: { file: "values_nobonus_nojoker.bin", rules: { extraYahtzeeBonus: 0, joker: false } },
};
...
export function App() {
  ...
  // Fetch the table for the selected mode the first time it is needed.
  useEffect(() => {
    if (engines[mode]) return;
    let cancelled = false;
    const { file, rules } = MODES[mode];
    fetch(file)
      .then((r) => { if (!r.ok) throw new Error(`${file} ${r.status}`); return r.arrayBuffer(); })
      .then((buf) => {
        if (cancelled) return;
        const built = buildEngine(new Float32Array(buf), rules);
        setEngines((prev) => ({ ...prev, [mode]: built }));
      })
  ...
  // Keyed by mode so changing the rules always starts a fresh game.
  return <Game key={mode} engine={engine} mode={mode} onModeChange={setMode} />;
}

const emptyCard = () => Array(13).fill(null);

function Game({ engine, mode, onModeChange }) {
  // reduced state
  ...
  const gameStarted = mask !== 0;      // rules lock once the first box is scored
  ...
  return (
    <div className="wrap">
      ...
      <section className="rulesbar">
        <div className="rulestop">
          <span className="ruleslabel">Repeat Yahtzee bonus</span>
          <div className="ruleschoice">
            <button type="button" className={"rulebtn" + (mode === "standard" ? " on" : "")}
              disabled={gameStarted} onClick={() => onModeChange("standard")}>On</button>
            <button type="button" className={"rulebtn" + (mode === "nobonus" ? " on" : "")}
              disabled={gameStarted} onClick={() => onModeChange("nobonus")}>Off</button>
          </div>
        </div>
        <p className="rulesnote muted">
          {mode === "standard"
            ? "Official rules. An extra Yahtzee scores 100 points, and the Joker rule decides where you can put it."
            : "An extra Yahtzee scores no bonus, and you can put it in any open box, where it counts as five ordinary dice."}
          {gameStarted ? " Locked once the game has started. Restart to change it." : ""}
        </p>
      </section>
...
function HowItWorks({ initialV }) {
  return (
    <section className="explainer">
      ...
      <p>Plenty of people don't play the repeat Yahtzee bonus. You can switch it off at the top of the Play
        tab, any time before you score your first box. The app then loads a second value table, worked out
        for a game where an extra Yahtzee earns no bonus and simply counts as five ordinary dice in any open
        box, so both the advice and the expected score match the rules you're playing.</p>
...
function Style() {
  return (
    ...
    * { box-sizing: border-box; }
    ...
    .rulesbar { background: #131a22; border: 1px solid #1f2a35; border-radius: 12px; padding: 10px 12px; margin-bottom: 12px; }
    .rulestop { display: flex; justify-content: space-between; align-items: center; gap: 10px; }
    .ruleslabel { font-size: .84rem; font-weight: 600; }
    .ruleschoice { display: flex; gap: 4px; }
    .rulebtn { padding: 4px 14px; font-size: .78rem; border-radius: 8px; border: 1px solid #2a3a49;
      background: #0f1720; color: #93a1b0; cursor: pointer; }
    .rulebtn.on { background: #12351f; border-color: #23a06b; color: #7fe6a0; font-weight: 700; }
    .rulebtn:disabled { cursor: default; opacity: .75; }
    .rulebtn:disabled:hover { border-color: #2a3a49; }
    .rulebtn.on:disabled:hover { border-color: #23a06b; }
    .rulesnote { margin: 6px 0 0; font-size: .74rem; line-height: 1.45; }
