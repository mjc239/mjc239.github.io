...
So, starting at the `|` tile below the `S`, having moved south from the `S`, move from tile to tile until reaching the `S` tile again, keeping count of the length of the loop so far:
...
The furthest point from the `S` tile is the point halfway around this loop (note that the `S` tile is excluded from `loop_length`):
...
Now that the loop has been found, we are now asked to find the number of tiles (either ground or unused pipe) inside the loop. To avoid having to handle extra special cases, it is helpful to replace the `S` tile with a `7` tile, as we know that this the true value:
