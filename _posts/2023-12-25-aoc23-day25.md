...
Great! `networkx` has a method called `connected_components` which determines the sizes of each of these clusters:
...
This matches exactly what was expected for the provided example - let's repeat for the full graph (first, re-initialise so it has the full set of edges):
